...
if ( ! empty( $args['schema'] ) ) {
    ...
    <ul class="cmswt-IndexSwitcher">
		...
		foreach ( $args['schema'] as $post_type => $collection ) {
			$label = cm_swt_get_label( $post_type, $args['config'] );
			if ( $i == 0 ) {
            ...
            <li class="cmswt-IndexSwitcher-item <?php echo ( 0 == $i ) ? 'active' : ''; ?>">
                <a href="#"
                   ...
                   data-collection="<?php echo esc_attr( TypesenseAPI::getInstance()->getCollectionNameFromSchema( $post_type ) ); ?>">
					<?php echo $label; ?>
                </a>
            </li>
			<?php
...
<div class="cmswt-CollectionMenu <?php echo $single_source_class; ?>">
	...
	foreach ( $args['passed_args']['post_types'] as $post_type ) {
		if ( $post_type === 'category' ) {
			...
			$sortByItems = apply_filters( 'cm_typesense_search_sortby_items', [
				[
					'label' => __( 'Recent', 'cm-news' ),
					'value' => TypesenseAPI::getInstance()->getCollectionNameFromSchema( $post_type )
				],
				[
					'label' => __( 'Oldest', 'cm-news' ),
